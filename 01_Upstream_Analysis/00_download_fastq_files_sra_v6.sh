...
############################################################
## Script: 00_download_fastq_files_sra_v6.sh
##
## Description:
## - Downloads SRR files listed in a text file
## - Converts them to FASTQ using fasterq-dump
## - Detects paired-end or single-end data
## - Compresses FASTQ files using pigz (mandatory)
## - Renames FASTQ files to Illumina-style naming at the end:
...
############################
mostrar_ayuda() {
    echo "Uso: $0 [-o OUTPUT_DIR] [-l SRR_FILE] <-h help>"
...
for SRR in $(cat "$SRR_FILE"); do
    ...
    if [ -f "${OUT_DIR}/${SRR}_2.fastq" ]; then
        ...
        pigz "${OUT_DIR}/${SRR}_1.fastq" "${OUT_DIR}/${SRR}_2.fastq"
